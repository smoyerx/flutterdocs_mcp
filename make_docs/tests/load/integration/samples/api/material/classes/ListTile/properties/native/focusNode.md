...
and managed by this widget. The widget will be focusable even if a [focusNode](flutter-docs://api/material/ListTile/focusNode) is not supplied. If supplied, the given [focusNode](flutter-docs://api/material/ListTile/focusNode) will be *hosted* by this widget, but not owned. See [FocusNode](flutter-docs://api/widgets/FocusNode) for more
...
responsible for calling [FocusNode.dispose](flutter-docs://api/widgets/FocusNode/dispose) on the focus node when it is
