...
For historical reasons, instead of using a `colorSchemeSeed` or `colorScheme`, you can provide either a `primaryColor` or `primarySwatch` to construct the `colorScheme`, but the results will not be as complete
...
will be used to override certain theme colors. The `colorScheme`, `textTheme`, `elevatedButtonTheme`, `outlinedButtonTheme`, `textButtonTheme`, `filledButtonTheme`, and `floatingActionButtonTheme` are overriden by the system colors.
